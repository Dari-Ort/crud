...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($nombre) && !empty($apellido) && !empty($telefono) && !empty($direccion) && !empty($observacion)) {
        ...
        $sql = "INSERT INTO tabla (nombre, apellido, telefono, direccion, observacion) VALUES (?, ?, ?, ?, ?)";
        ...
        if ($stmt = mysqli_prepare($conn, $sql)) {
            ...
            mysqli_stmt_bind_param($stmt, "sssss", $nombre, $apellido, $telefono ,$direccion, $observacion);
