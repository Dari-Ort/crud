...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($nombre) && !empty($apellido) && !empty($telefono) && !empty($direccion) && !empty($observacion)) {
        // Preparar la consulta SQL para actualizar los datos del registro
        $sql = "UPDATE tabla SET nombre = ?, apellido = ?, telefono = ?, direccion = ?, observacion = ? WHERE id = ?";
        
        // Preparar la sentencia para actualizar el registro
        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Vincular los parámetros a la consulta (nombre, dirección, observación, ID)
            mysqli_stmt_bind_param($stmt, "sssssi", $nombre, $apellido, $telefono, $direccion, $observacion, $id);
...
<!-- este es el script que hace el aviso de si quieres editar la informacion  -->
<script>
    function confi(){
        var respu = confirm("¿Deseas editar la informacion ?");
        if(respu == true ){
            return true;
        }else{
            return false;
            ...
            <div class="card-body">
                ...
                <form action="editar.php" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input  minlength="4" maxlength="10" type="text" class="form-control" id="telefono" name="telefono" value="<?php echo $row['telefono']; ?>" required pattern = "[0-9]+">
                    </div>
                    ...
                    <div class="d-grid gap-2">
                        <!-- aca se pone onclick para que aparezca cuando lo undas y el return es para que solamente se ejecute cuando le unda para actualizar el edit -->
                        <button type="submit" class="btn btn-primary" onclick="return confi()">Actualizar</button>
